...
$ bash getbstates.sh
~~~
Five basis states can then be chosen from the 25 options generated by the script (five for each run). For example, a random number generator can be used to generate five random numbers between 1 and 25. Place the selected basis states in the /.../we_simulations/bstates/ folder. The associated pcoord.init file will be generated when the simulation initializes.
