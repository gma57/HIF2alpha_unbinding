...
class IterationProcessor(object):
    ...
    def process_iteration(self, n_iter, iter_group):
        ...
        for iseg in range(nsegs):
            ...
            try:
                os.symlink(surf_file, pathy+'/cpptraj_ene.in')
            except FileExistsError:
                os.unlink(pathy+'/cpptraj_ene.in')
                os.symlink(surf_file, pathy+'/cpptraj_ene.in')
            #try:
            ...
            command = 'cpptraj -i cpptraj_ene.in > /dev/null '
            ...
            x1,x2,x3 = numpy.loadtxt(f"{pathy}/ene.dat", skiprows=1, usecols=(3,6,9), unpack=True)
            ...
            #c = y1 + y2 + y3
            #e = b + c
            iteration_data_array[0][iseg, :] = arr[:,0]
            iteration_data_array[1][iseg, :] = arrN[:,0] 
            ...
            with open('/ix/lchong/ris130/WESTPA/new_simulations_unb/unb1_MULTIMAB_rates/crawl_energies/update.txt', 'a') as fo:
...
class Crawler(WESTPACrawler):
    ...
    def process_iter_result(self, n_iter, result):
        ...
        # Save datasets
        dataset = iter_group.create_dataset('iter_energy_P', 
                                            data=iteration_data_array[0], 
                                            scaleoffset=6, 
                                            compression=4,
                                            chunks=h5io.calc_chunksize(
                                                    iteration_data_array[0].shape,
                                                    iteration_data_array[0].dtype
                                                                       )
                                            )
